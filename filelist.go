...
func displayFileContent(g *gocui.Gui, app *AppState, filePath string) error {
	...
	// Check if file has valid file_url (was generated with API key)
	if !isValidFileURL(match.FileURL) {
		fmt.Fprintf(v, "File Content Not Available\n")
		fmt.Fprintf(v, "========================\n\n")
		fmt.Fprintf(v, "Content cannot be displayed because this JSON file\n")
		fmt.Fprintf(v, "was generated without an API key.\n\n")
		fmt.Fprintf(v, "To enable file content viewing:\n")
		fmt.Fprintf(v, "• Regenerate the scan using an API key\n\n")
...
func isValidFileURL(fileURL string) bool {
	trimmed := strings.TrimSpace(fileURL)
	return trimmed != "" && strings.HasPrefix(trimmed, "https://")
}
